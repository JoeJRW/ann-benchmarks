...
def many_queries(arg):
    ...
    res = []
    for v in arg[4]:
        cur.execute(f"SELECT id FROM t1 ORDER by DISTANCE(v, %s) LIMIT %s", (vector_to_hex(v), arg[3]))
...
class Heatwave(BaseANN):
    ...
    def prepare_options(self):
        self._perf_stat = os.environ.get('PERF', 'no') == 'yes' and Heatwave.can_run_perf()
        self._perf_record = os.environ.get('FLAMEGRAPH', 'no') == 'yes' and Heatwave.can_run_flamegraph()
        ...
        user_option = Heatwave.get_user_option()
    ...
    def fit(self, X):
        ...
        if self._batch:
            ...
            for i, embedding in enumerate(X):
                icur.execute("INSERT INTO t1 (id, v) VALUES (%s, UNHEX(%s))", (i, vector_to_hex(embedding)))
                if i - rows > rps:
                    now=time.time()
    ...
    def query(self, v, n):
        ...
        self._cur.execute(f"SELECT id FROM t1 ORDER by DISTANCE(v, UNHEX(%s), %s) LIMIT %s", (vector_to_hex(v), self._metric, n))
